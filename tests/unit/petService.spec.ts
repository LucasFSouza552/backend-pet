...
describe('PetService', () => {
  ...
  describe('acceptRequestedAdoption', () => {
    ...
    it('deve falhar quando histórico não existe ao aceitar adoção', async () => {
      const petData = createMockPet({ account: 'institution123' });
      petRepository.getById.mockResolvedValue(petData);
      historyRepository.getPendingByAccountAndPet.mockResolvedValue(null);

      await expect(service.acceptRequestedAdoption('pet123', 'user123', 'institution123'))
        .rejects.toThrow('Histórico não encontrado.');
    });

    it('deve falhar quando histórico já foi processado', async () => {
      const petData = createMockPet({ account: 'institution123' });
      const history = {
        id: 'history123',
        status: 'completed'
      ...
      petRepository.getById.mockResolvedValue(petData);
      historyRepository.getPendingByAccountAndPet.mockResolvedValue(history);
  ...
  describe('softDelete', () => {
    ...
    it('deve impedir deletar pet já adotado', async () => {
      const petData = createMockPet({ account: 'owner123', adopted: true });
      petRepository.getById.mockResolvedValue(petData);

      await expect(service.softDelete('pet123', 'owner123'))
        .rejects.toThrow('O Pet já foi adotado.');
    });
  });
});
